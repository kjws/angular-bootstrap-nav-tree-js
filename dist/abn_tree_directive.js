    ...
    '$timeout', function($timeout) {
      return {
        ...
        template:
          "<ul class=\"nav nav-list nav-pills nav-stacked abn-tree\">\n  "+
            "<li ng-repeat=\"row in tree_rows | filter:{visible:true} track by row.branch.uid\" ng-animate=\"'abn-tree-animate'\" ng-class=\"'level-' + {{ row.level }} + (row.branch.selected||user_set_active(row.branch) ? ' active':'')\" class=\"abn-tree-row\">\n    "+
              "<a ng-drop='{{!row.branch.data}}' ng-drag=true ng-drag-data=row.branch ng-center-anchor='true' ng-drop-success='onDropEnd(row.branch,$data,$event)' id='{{row.branch.label}}' href='javascript:void(0);' ng-init='hover=false' ng-mouseenter='hover=true' ng-mouseleave='hover=false' ng-click=\"user_clicks_branch($event, row.branch)\">\n      "+
        ...
        link: function(scope, element, attrs) {
          ...
          scope.onDropEnd = function (p, c, e) {
            scope.treeControl.onDropEnd(p, c, e);
          };
